...
public class SimpleValidateQueryTests extends ElasticsearchIntegrationTest {
    ...
    private String indexedValueForSearchAsString(int value) {
        BytesRefBuilder bytesRef = new BytesRefBuilder();
        NumericUtils.intToPrefixCoded(value, 0, bytesRef); // 0 because of exact match
        return Term.toString(bytesRef.get());
    }
    ...
    public void explainValidateQuery() throws Exception {
        ...
        assertExplanation(QueryBuilders.filteredQuery(QueryBuilders.termQuery("foo", "1"), FilterBuilders.orFilter(FilterBuilders.termFilter("bar", "2"), FilterBuilders.termFilter("baz", "3"))), equalTo("filtered(filtered(foo:1)->" + filter("bar:" + indexedValueForSearchAsString(2)) + " " + filter("baz:3") + ")->" + typeFilter));

        assertExplanation(QueryBuilders.filteredQuery(QueryBuilders.termQuery("foo", "1"), FilterBuilders.orFilter(FilterBuilders.termFilter("bar", "2"))), equalTo("filtered(filtered(foo:1)->" + filter("bar:" + indexedValueForSearchAsString(2)) + ")->" + typeFilter));

        assertExplanation(QueryBuilders.filteredQuery(QueryBuilders.matchAllQuery(), FilterBuilders.geoPolygonFilter("pin.location").addPoint(40, -70).addPoint(30, -80).addPoint(20, -90).addPoint(40, -70) // closing polygon
        ...
        assertExplanation(QueryBuilders.filteredQuery(QueryBuilders.termQuery("foo", "1"), FilterBuilders.andFilter(FilterBuilders.termFilter("bar", "2"), FilterBuilders.termFilter("baz", "3"))), equalTo("filtered(filtered(foo:1)->+" + filter("bar:" + indexedValueForSearchAsString(2)) + " +" + filter("baz:3") + ")->" + typeFilter));
    ...
    public void explainDateRangeInQueryString() {
        assertAcked(prepareCreate("test").setSettings(ImmutableSettings.settingsBuilder().put(indexSettings()).put("index.number_of_shards", 1)));
        ...
        ValidateQueryResponse response = client().admin().indices().prepareValidateQuery().setQuery(queryStringQuery("past:[now-2M/d TO now/d]")).setExplain(true).get();
    ...
    public void explainFilteredAlias() {
        assertAcked(prepareCreate("test").addMapping("test", "field", "type=string").addAlias(new Alias("alias").filter(FilterBuilders.termFilter("field", "value1"))));
        ensureGreen();
    ...
    public void explainWithRewriteValidateQuery() throws Exception {
        ...
        assertExplanation(QueryBuilders.commonTermsQuery("field", "huge brown pidgin").cutoffFrequency(1), containsString("(field:huge field:brown) +field:pidgin"), true);
        ...
        assertExplanation(QueryBuilders.matchQuery("field", "huge brown pidgin").cutoffFrequency(1), containsString("(field:huge field:brown) +field:pidgin"), true);
        ...
        assertExplanation(QueryBuilders.moreLikeThisQuery("field").ids("1").include(true).minTermFreq(1).minDocFreq(1).maxQueryTerms(2), containsString("field:huge field:pidgin"), true);
        assertExplanation(QueryBuilders.moreLikeThisQuery("field").likeText("the huge pidgin").minTermFreq(1).minDocFreq(1).maxQueryTerms(2), containsString("field:huge field:pidgin"), true);
    }
