...
public class SimpleDateMappingTests extends ElasticsearchSingleNodeTest {
    ...
    public void testAutomaticDateParser() throws Exception {
        ...
        defaultMapper.parse("type", "1", XContentFactory.jsonBuilder().startObject().field("date_field1", "2011/01/22").field("date_field2", "2011/01/22 00:00:00").field("wrong_date1", "-4").field("wrong_date2", "2012/2").field("wrong_date3", "2012/test").endObject().bytes());
    ...
    public void testLocale() throws IOException {
        ...
        DocumentMapper defaultMapper = mapper(mapping);
        ParsedDocument doc = defaultMapper.parse("type", "1", XContentFactory.jsonBuilder().startObject().field("date_field_en", "Wed, 06 Dec 2000 02:55:00 -0800").field("date_field_de", "Mi, 06 Dez 2000 02:55:00 -0800").field("date_field_default", "Wed, 06 Dec 2000 02:55:00 -0800") // check default - no exception is a successs!
                        .endObject().bytes());
    ...
    public void testTimestampAsDate() throws Exception {
        ...
        ParsedDocument doc = defaultMapper.parse("type", "1", XContentFactory.jsonBuilder().startObject().field("date_field", value).endObject().bytes());
    ...
    public void testDateDetection() throws Exception {
        ...
        ParsedDocument doc = defaultMapper.parse("type", "1", XContentFactory.jsonBuilder().startObject().field("date_field", "2010-01-01").field("date_field_x", "2010-01-01").endObject().bytes());
    ...
    public void testNumericResolution() throws Exception {
        String mapping = XContentFactory.jsonBuilder().startObject().startObject("type").startObject("properties").startObject("date_field").field("type", "date").field("format", "date_time").field("numeric_resolution", "seconds").endObject().endObject().endObject().endObject().string();

        DocumentMapper defaultMapper = mapper(mapping);

        // provided as an int
        ParsedDocument doc = defaultMapper.parse("type", "1", XContentFactory.jsonBuilder().startObject().field("date_field", 42).endObject().bytes());
        assertThat(getDateAsMillis(doc.rootDoc(), "date_field"), equalTo(42000L));

        // provided as a string
        doc = defaultMapper.parse("type", "2", XContentFactory.jsonBuilder().startObject().field("date_field", "43").endObject().bytes());
        assertThat(getDateAsMillis(doc.rootDoc(), "date_field"), equalTo(43000L));

        // but formatted dates still parse as milliseconds
        doc = defaultMapper.parse("type", "2", XContentFactory.jsonBuilder().startObject().field("date_field", "1970-01-01T00:00:44.000Z").endObject().bytes());
