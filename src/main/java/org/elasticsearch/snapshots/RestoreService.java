...
public class RestoreService extends AbstractComponent implements ClusterStateListener {
    ...
    private static final ImmutableSet<String> UNMODIFIABLE_SETTINGS = ImmutableSet.of(SETTING_NUMBER_OF_SHARDS, SETTING_VERSION_CREATED, SETTING_UUID, SETTING_CREATION_DATE);

    // It's OK to change some settings, but we shouldn't allow simply removing them
    private static final ImmutableSet<String> UNREMOVABLE_SETTINGS = ImmutableSet.<String> builder().addAll(UNMODIFIABLE_SETTINGS).add(SETTING_NUMBER_OF_REPLICAS).add(SETTING_AUTO_EXPAND_REPLICAS).add(SETTING_VERSION_UPGRADED).add(SETTING_VERSION_MINIMUM_COMPATIBLE).build();
                    ...
                    if (!renamedIndices.isEmpty()) {
                        ...
                        for (Map.Entry<String, String> indexEntry : renamedIndices.entrySet()) {
                            ...
                            if (currentIndexMetaData == null) {
                                ...
                                createIndexService.validateIndexSettings(renamedIndex, snapshotIndexMetaData.settings(), currentState);
    ...
    public void indexShardRestoreCompleted(SnapshotId snapshotId, ShardId shardId) {
        ...
        if (clusterService.state().nodes().localNodeMaster()) {
            ...
            transportService.sendRequest(clusterService.state().nodes().masterNode(), UPDATE_RESTORE_ACTION_NAME, request, EmptyTransportResponseHandler.INSTANCE_SAME);
        }
    }
                    ...
                    for (RestoreMetaData.Entry entry : restore.entries()) {
                        ...
                        if (shards != null) {
                            if (!completed(shards)) {
                                ...
                                batchedRestoreInfo.put(entry.snapshotId(), new Tuple<>(new RestoreInfo(entry.snapshotId().getSnapshot(), entry.indices(), shards.size(), shards.size() - failedShards(shards)), shards));
                            }
                        } else {
                            entries.add(entry);
                        }
    ...
    public void failRestore(SnapshotId snapshotId, ShardId shardId) {
        ...
        UpdateIndexShardRestoreStatusRequest request = new UpdateIndexShardRestoreStatusRequest(snapshotId, shardId, new ShardRestoreStatus(clusterService.state().nodes().localNodeId(), RestoreMetaData.State.FAILURE));
        if (clusterService.state().nodes().localNodeMaster()) {
            innerUpdateRestoreState(request);
        } else {
            transportService.sendRequest(clusterService.state().nodes().masterNode(), UPDATE_RESTORE_ACTION_NAME, request, EmptyTransportResponseHandler.INSTANCE_SAME);
