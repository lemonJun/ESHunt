...
public class TransportClientNodesService extends AbstractComponent {
    ...
    @Inject
    public TransportClientNodesService(Settings settings, ClusterName clusterName, TransportService transportService, ThreadPool threadPool, Headers headers, Version version) {
        super(settings);
    ...
    public static class RetryListener<Response> implements ActionListener<Response> {
        ...
        public RetryListener(NodeListenerCallback<Response> callback, ActionListener<Response> listener, ImmutableList<DiscoveryNode> nodes, int index, ThreadPool threadPool, ESLogger logger) {
    ...
    class SimpleNodeSampler extends NodeSampler {
        ...
        protected void doSample() {
            ...
            for (DiscoveryNode listedNode : listedNodes) {
                ...
                try {
                    NodesInfoResponse nodeInfo = transportService.submitRequest(listedNode, NodesInfoAction.NAME, headers.applyTo(Requests.nodesInfoRequest("_local").clear()), TransportRequestOptions.options().withType(TransportRequestOptions.Type.STATE).withTimeout(pingTimeout), new FutureTransportResponseHandler<NodesInfoResponse>() {
                        @Override
