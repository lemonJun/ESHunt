...
public abstract class AbstractIndicesAdminClient implements IndicesAdminClient {
                    ...
                    try {
                        ...
                        for (String feature : features) {
                            switch (feature) {
                                case "_alias":
                                case "_aliases":
                                    builderList.add(prepareGetAliases(new String[0]).addIndices(request.indices()).setIndicesOptions(request.indicesOptions()));
                                    break;
                                case "_mapping":
                                case "_mappings":
                        ...
                        final ActionListener<?> actionListener = new ActionListener<Object>() {
                            ...
                            public void onResponse(Object o) {
                                ...
                                if (countDown.countDown()) {
                                    GetIndexResponse response = GetIndexResponse.convertResponses(aliasResponse, mappingResponse, settingsResponse, warmerResponse);
                                    listener.onResponse(response);
                                }
                            }
    ...
    public UpgradeRequestBuilder prepareUpgrade(String... indices) {
        return new UpgradeRequestBuilder(this).setIndices(indices);
    }
    ...
    public UpgradeStatusRequestBuilder prepareUpgradeStatus(String... indices) {
        return new UpgradeStatusRequestBuilder(this).setIndices(indices);
    }
