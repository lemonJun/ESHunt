    ...
    public void clusterChanged(ClusterChangedEvent event) {
        ...
        if (nrReplicasChanged.size() > 0) {
            for (final Integer fNumberOfReplicas : nrReplicasChanged.keySet()) {
                ...
                UpdateSettingsClusterStateUpdateRequest updateRequest = new UpdateSettingsClusterStateUpdateRequest().indices(indices.toArray(new String[indices.size()])).settings(settings).ackTimeout(TimeValue.timeValueMillis(0)) //no need to wait for ack here
        ...
        clusterService.submitStateUpdateTask("update-settings", Priority.URGENT, new AckedClusterStateUpdateTask<ClusterStateUpdateResponse>(request, listener) {
            ...
            public ClusterState execute(ClusterState currentState) {
                ...
                if (closeIndices.size() > 0 && closeSettings.get(IndexMetaData.SETTING_NUMBER_OF_REPLICAS) != null) {
                    throw new ElasticsearchIllegalArgumentException(String.format(Locale.ROOT, "Can't update [%s] on closed indices [%s] - can leave index in an unopenable state", IndexMetaData.SETTING_NUMBER_OF_REPLICAS, closeIndices));
                }
            ...
            public ClusterState execute(ClusterState currentState) {
                ...
                for (Map.Entry<String, String> entry : request.versions().entrySet()) {
                    ...
                    if (indexMetaData != null) {
                        if (Version.CURRENT.equals(Version.indexCreated(indexMetaData.settings())) == false) {
