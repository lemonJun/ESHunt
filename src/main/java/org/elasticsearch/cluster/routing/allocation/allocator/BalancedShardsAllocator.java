            ...
            if (!changed) {
                ...
                for (ModelNode currentNode : nodes) {
                    ...
                    if (decision.type() == Type.YES) { // TODO maybe we can respect throttling here too?
                        ...
                        final MutableShardRouting initializingShard = new MutableShardRouting(shard.index(), shard.id(), currentNode.getNodeId(), shard.currentNodeId(), shard.restoreSource(), shard.primary(), INITIALIZING, shard.version() + 1);
                ...
                if (candidate != null) {
                    ...
                    if (decision.type() == Type.YES) { /* only allocate on the cluster if we are not throttled */
                        ...
                        if (candidate.started()) {
                            ...
                            routingNodes.assign(new MutableShardRouting(candidate.index(), candidate.id(), lowRoutingNode.nodeId(), candidate.currentNodeId(), candidate.restoreSource(), candidate.primary(), INITIALIZING, candidate.version() + 1), lowRoutingNode.nodeId());
