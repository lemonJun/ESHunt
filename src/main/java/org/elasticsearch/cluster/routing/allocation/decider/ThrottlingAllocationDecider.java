...
public class ThrottlingAllocationDecider extends AllocationDecider {
        ...
        if (shardRouting.primary()) {
            ...
            if (shardRouting.unassigned()) {
                ...
                if (primariesInRecovery >= primariesInitialRecoveries) {
                    return allocation.decision(Decision.THROTTLE, NAME, "too many primaries currently recovering [%d], limit: [%d]", primariesInRecovery, primariesInitialRecoveries);
                } else {
    ...
    public Decision canAllocate(RoutingNode node, RoutingAllocation allocation) {
        ...
        if (currentRecoveries >= concurrentRecoveries) {
            return allocation.decision(Decision.THROTTLE, NAME, "too many shards currently recovering [%d], limit: [%d]", currentRecoveries, concurrentRecoveries);
        } else {
