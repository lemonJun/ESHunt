...
public class NodeEnvironment extends AbstractComponent implements Closeable {
    ...
    private void maybeLogPathDetails() throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.info(String.format(Locale.ROOT, "using [%d] data paths, mounts [%s], net usable_space [%s], net total_space [%s], types [%s]", nodePaths.length, allMounts, totFSInfo.getAvailable(), totFSInfo.getTotal(), toString(allTypes)));
        }
    }
    ...
    public Path[] shardPaths(ShardId shardId) {
        ...
        for (int i = 0; i < nodePaths.length; i++) {
            shardLocations[i] = nodePaths[i].path.resolve(Paths.get(INDICES_FOLDER, shardId.index().name(), Integer.toString(shardId.id())));
        }
    ...
    private static Set<ShardId> findAllShardIds(@Nullable
    final String index, Path... locations) throws IOException {
    ...
    public void ensureAtomicMoveSupported() throws IOException {
        ...
        for (NodePath nodePath : nodePaths) {
            ...
            try {
                ...
                throw new ElasticsearchIllegalStateException("atomic_move is not supported by the filesystem on path [" + nodePath.path + "] atomic_move is required for elasticsearch to work correctly.", ex);
            } finally {
