...
public abstract class ShapeBuilder implements ToXContent {
     ...
     * @return a new {@link EnvelopeBuilder}
     */
    public static EnvelopeBuilder newEnvelope(Orientation orientation) {
    ...
    protected static final class Edge {
         ...
         */
        protected static Edge[] ring(int component, boolean direction, boolean handedness, BaseLineStringBuilder<?> shell, Coordinate[] points, int offset, Edge[] edges, int toffset, int length) {
    ...
    public static enum GeoShapeType {
        POINT("point"), MULTIPOINT("multipoint"), LINESTRING("linestring"), MULTILINESTRING("multilinestring"), POLYGON("polygon"), MULTIPOLYGON("multipolygon"), GEOMETRYCOLLECTION("geometrycollection"), ENVELOPE("envelope"), CIRCLE("circle");
        ...
        protected static LineStringBuilder parseLineString(CoordinateNode coordinates) {
             ...
             */
            if (coordinates.children.size() < 2) {
                throw new ElasticsearchParseException("Invalid number of points in LineString (found " + coordinates.children.size() + " - must be >= 2)");
            }
