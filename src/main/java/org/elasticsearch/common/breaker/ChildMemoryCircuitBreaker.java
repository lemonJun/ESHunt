...
public class ChildMemoryCircuitBreaker implements CircuitBreaker {
     ...
     */
    public ChildMemoryCircuitBreaker(BreakerSettings settings, ESLogger logger, HierarchyCircuitBreakerService parent, Name name) {
     ...
     */
    public ChildMemoryCircuitBreaker(BreakerSettings settings, ChildMemoryCircuitBreaker oldBreaker, ESLogger logger, HierarchyCircuitBreakerService parent, Name name) {
        this.name = name;
        ...
        if (this.memoryBytesLimit == -1) {
            ...
            do {
                ...
                if (memoryBytesLimit > 0 && newUsedWithOverhead > memoryBytesLimit) {
                    logger.warn("[{}] New used memory {} [{}] from field [{}] would be larger than configured breaker: {} [{}], breaking", this.name, newUsedWithOverhead, new ByteSizeValue(newUsedWithOverhead), label, memoryBytesLimit, new ByteSizeValue(memoryBytesLimit));
