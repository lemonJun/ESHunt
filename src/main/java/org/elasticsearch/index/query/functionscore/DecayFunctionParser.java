...
public abstract class DecayFunctionParser implements ScoreFunctionParser {
    ...
    private AbstractDistanceScoreFunction parseNumberVariable(String fieldName, XContentParser parser, QueryParseContext parseContext, NumberFieldMapper<?> mapper, MultiValueMode mode) throws IOException {
        ...
        if (!scaleFound || !refFound) {
            throw new ElasticsearchParseException("Both " + DecayFunctionBuilder.SCALE + " and " + DecayFunctionBuilder.ORIGIN + " must be set for numeric fields.");
        }
    ...
    public static abstract class AbstractDistanceScoreFunction extends ScoreFunction {
        ...
        public AbstractDistanceScoreFunction(double userSuppiedScale, double decay, double offset, DecayFunction func, MultiValueMode mode) {
            ...
            if (decay <= 0.0 || decay >= 1.0) {
                throw new ElasticsearchIllegalArgumentException(FunctionScoreQueryParser.NAME + " : decay must be in the range [0..1].");
            }
