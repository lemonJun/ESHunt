...
public class Store extends AbstractIndexShardComponent implements CloseableIndexComponent, Closeable, RefCounted {
    ...
    static boolean isUnreliableLegacyChecksum(StoreFileMetaData metadata) {
        if (metadata.hasLegacyChecksum()) {
            ...
            boolean badCFS = metadata.name().endsWith(".cfs") && (metadata.writtenBy() == null || metadata.writtenBy().onOrAfter(Version.LUCENE_34) == false);
        ...
        try (Lock writeLock = Lucene.acquireWriteLock(directory)) {
            ...
            for (String existingFile : dir.listAll()) {
                ...
                try {
                    ...
                    if (existingFile.startsWith(IndexFileNames.SEGMENTS) || existingFile.equals(IndexFileNames.SEGMENTS_GEN)) {
    ...
    public final static class MetadataSnapshot implements Iterable<StoreFileMetaData>, Streamable {
        ...
        public RecoveryDiff recoveryDiff(MetadataSnapshot recoveryTargetSnapshot) {
            ...
            assert recoveryDiff.size() == this.metadata.size() - (metadata.containsKey(IndexFileNames.SEGMENTS_GEN) ? 1 : 0) : "some files are missing recoveryDiff size: [" + recoveryDiff.size() + "] metadata size: [" + this.metadata.size() + "] contains  segments.gen: [" + metadata.containsKey(IndexFileNames.SEGMENTS_GEN) + "]";
            return recoveryDiff;
        }
    ...
    static class LuceneVerifyingIndexOutput extends VerifyingIndexOutput {
        ...
        private void readAndCompareChecksum() throws IOException {
            ...
            if (!metadata.checksum().equals(actualChecksum)) {
                throw new CorruptIndexException("checksum failed (hardware problem?) : expected=" + metadata.checksum() + " actual=" + actualChecksum + " (resource=" + metadata.toString() + ")");
            }
        }
    ...
    static class VerifyingIndexInput extends ChecksumIndexInput {
        ...
        public long verify() throws CorruptIndexException {
            ...
            throw new CorruptIndexException("verification failed : calculated=" + Store.digestToString(getChecksum()) + " stored=" + Store.digestToString(storedChecksum));
        }
