...
public abstract class NumberFieldMapper<T extends Number> extends AbstractFieldMapper<T> implements AllFieldMapper.IncludeInAll {
    ...
    protected NumberFieldMapper(Names names, int precisionStep, float boost, FieldType fieldType, Boolean docValues, Explicit<Boolean> ignoreMalformed, Explicit<Boolean> coerce, NamedAnalyzer indexAnalyzer, NamedAnalyzer searchAnalyzer, PostingsFormatProvider postingsProvider, DocValuesFormatProvider docValuesProvider, SimilarityProvider similarity, Loading normsLoading, @Nullable Settings fieldDataSettings, Settings indexSettings, MultiFields multiFields, CopyTo copyTo) {
    ...
    protected void parseCreateField(ParseContext context, List<Field> fields) throws IOException {
        ...
        try {
            innerParseCreateField(context, fields);
                        ...
                        | ElasticsearchIllegalArgumentException e1) {
            e = e1;
    ...
    public Filter fieldDataTermsFilter(List values, @Nullable QueryParseContext context) {
        ...
        if (fieldData.getNumericType().isFloatingPoint()) {
            ...
            DoubleOpenHashSet terms = new DoubleOpenHashSet((int) (values.size() * (1 + DoubleOpenHashSet.DEFAULT_LOAD_FACTOR)));
            ...
            // create with initial size large enough to avoid rehashing
            LongOpenHashSet terms = new LongOpenHashSet((int) (values.size() * (1 + LongOpenHashSet.DEFAULT_LOAD_FACTOR)));
            for (int i = 0, len = values.size(); i < len; i++) {
