...
public class UpdateHelper extends AbstractComponent {
    ...
    public Result prepare(UpdateRequest request, IndexShard indexShard) {
        ...
        if (!getResult.isExists()) {
            ...
            indexRequest.index(request.index()).type(request.type()).id(request.id())
                            ...
                            .create(true).ttl(ttl).refresh(request.refresh()).routing(request.routing()).parent(request.parent()).replicationType(request.replicationType()).consistencyLevel(request.consistencyLevel());
     ...
     */
    public GetResult extractGetResult(final UpdateRequest request, String concreteIndex, long version, final Map<String, Object> source, XContentType sourceContentType, @Nullable
