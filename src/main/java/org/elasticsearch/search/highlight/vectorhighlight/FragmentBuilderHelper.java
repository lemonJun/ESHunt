...
public final class FragmentBuilderHelper {
    ...
    public static WeightedFragInfo fixWeightedFragInfo(FieldMapper<?> mapper, Field[] values, WeightedFragInfo fragInfo) {
        ...
        if (!fragInfo.getSubInfos().isEmpty() && (containsBrokenAnalysis(mapper.indexAnalyzer()))) {
            ...
            return new WeightedFragInfo(Math.min(fragInfo.getSubInfos().get(0).getTermsOffsets().get(0).getStartOffset(), fragInfo.getStartOffset()), fragInfo.getEndOffset(), subInfos, fragInfo.getTotalBoost());
        } else {
            return fragInfo;
        }
    }
    ...
    private static boolean containsBrokenAnalysis(Analyzer analyzer) {
        ...
        if (analyzer instanceof CustomAnalyzer) {
            ...
            for (TokenFilterFactory tokenFilterFactory : tokenFilters) {
                ...
                if (tokenFilterFactory instanceof NGramTokenFilterFactory && !((NGramTokenFilterFactory) tokenFilterFactory).version().onOrAfter(Version.LUCENE_42)) {
