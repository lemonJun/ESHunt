...
public class RangeAggregator extends BucketsAggregator {
    ...
    public RangeAggregator(String name, AggregatorFactories factories, ValuesSource.Numeric valuesSource, @Nullable ValueFormat format, InternalRange.Factory rangeFactory, List<Range> ranges, boolean keyed, AggregationContext aggregationContext, Aggregator parent) {
    ...
    public InternalAggregation buildAggregation(long owningBucketOrdinal) {
        ...
        for (int i = 0; i < ranges.length; i++) {
            ...
            org.elasticsearch.search.aggregations.bucket.range.Range.Bucket bucket = rangeFactory.createBucket(range.key, range.from, range.to, bucketDocCount(bucketOrd), bucketAggregations(bucketOrd), formatter);
            buckets.add(bucket);
        }
    ...
    public InternalAggregation buildEmptyAggregation() {
        ...
        for (int i = 0; i < ranges.length; i++) {
            Range range = ranges[i];
            org.elasticsearch.search.aggregations.bucket.range.Range.Bucket bucket = rangeFactory.createBucket(range.key, range.from, range.to, 0, subAggs, formatter);
            buckets.add(bucket);
        }
        // value source can be null in the case of unmapped fields
        return rangeFactory.create(name, buckets, formatter, keyed);
