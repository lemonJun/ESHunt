...
public class OrderPath {
    ...
    public double resolveValue(HasAggregations root) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            if (i != tokens.length - 1) {
                throw new ElasticsearchIllegalArgumentException("Invalid order path [" + this + "]. Metrics aggregations cannot have sub-aggregations (at [" + token + ">" + tokens[i + 1] + "]");
            }
    ...
    public Aggregator resolveAggregator(Aggregator root) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            assert (aggregator instanceof SingleBucketAggregator && i <= tokens.length - 1) || (aggregator instanceof NumericMetricsAggregator && i == tokens.length - 1) : "this should be picked up before aggregation execution - on validate";
        }
        return aggregator;
    }
