...
public class IndexFieldTerm implements Iterable<TermPosition> {
            ...
            if (terms != null) {
                if (terms.hasPositions()) {
                    ...
                    if (termsEnum.seekExact(identifier.bytes())) {
                        newDocsEnum = termsEnum.docsAndPositions(reader.getLiveDocs(), docsEnum instanceof DocsAndPositionsEnum ? (DocsAndPositionsEnum) docsEnum : null, luceneFlags);
                    }
                }
            }
        }
    ...
    public IndexFieldTerm(String term, String fieldName, IndexLookup indexLookup, int flags) {
        ...
        try {
            termStats = indexLookup.getIndexSearcher().termStatistics(identifier, TermContext.build(indexLookup.getReaderContext(), identifier));
        } catch (IOException e) {
