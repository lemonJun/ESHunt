...
public class RecoveriesCollection {
    ...
    public long startRecovery(IndexShard indexShard, DiscoveryNode sourceNode, RecoveryTarget.RecoveryListener listener, TimeValue activityTimeout) {
        ...
        threadPool.schedule(activityTimeout, ThreadPool.Names.GENERIC, new RecoveryMonitor(status.recoveryId(), status.lastAccessTime(), activityTimeout));
    ...
    private class RecoveryMonitor extends AbstractRunnable {
        ...
        protected void doRun() throws Exception {
            ...
            if (accessTime == lastSeenAccessTime) {
                ...
                failRecovery(recoveryId, new RecoveryFailedException(status.state(), message, new ElasticsearchTimeoutException(message)), true // to be safe, we don't know what go stuck
                );
                return;
            }
